...
class Runner:
    ...
    def run(self):
        ...
        with open('sorted_distances.csv', mode='w', newline='') as file:
            ...
            # 写入表头
            writer.writerow(['rank', 'sorted_distance', 'sorted_idx'])

            # 将每个排序的距离和它们的排名保存
            for rank, (dist, idx) in enumerate(zip(sorted_distance, sorted_indices)):
                writer.writerow([rank + 1, dist.item(), idx.item()])  # rank + 1 表示排名从1开始
        ...
        #             break
                # torch.save(self.model.state_dict(), "MEAformer_model_parameters_FBYG15K_0.2.pth")
    ...
    def _test(self, test_left, test_right, last_epoch=False, save_name="", loss=None):
        with torch.no_grad():
            ...
            if self.args.model_name in ["MEAformer"]:
                ...
                weight_norm = None
            final_emb = F.normalize(final_emb)
        print('weight_norm', weight_norm[13])
        #
        # weight_norm = weight_norm.cpu().tolist()
        # with open('FBYG15k_weight_norms.csv', mode='w', newline='') as file:
        #     writer = csv.writer(file)
        #
        #     # 写入表头
        #     writer.writerow(['idx', 'img', 'attr', 'rel', 'graph'])
        #
        #     # 遍历 weight_norm，每一项是一个列表，例如 [img, attr, rel, graph]
        #     for idx, norm_values in enumerate(weight_norm):
        #         # 确保 norm_values 是一个长度为 4 的列表
        #         if len(norm_values) == 4:
        #             writer.writerow([idx] + norm_values)  # 将 idx 和 norm_values 合并成一行写入文件
        #         else:
        #             print(f"Warning: Data at idx {idx} is not of length 4, skipping.")
        #
        # # 输出完成提示
        # print('CSV 文件已保存为 "weight_norms.csv"')
